...
# For Raspberry Pi 5 (16GB) with AI HAT+ 2
#
# Prerequisites:
#   - Raspberry Pi OS (64-bit, Bookworm)
#   - AI HAT+ 2 drivers installed
#   - Network configured
#
# Usage:
#   chmod +x install.sh
...
create_config() {
    ...
    echo "If Ollama runs on a different machine, enter its IP address."
    echo "Press Enter to use localhost (same machine)."
    read -p "Ollama host [localhost]: " OLLAMA_INPUT
...
setup_ollama() {
    ...
    if check_command ollama; then
        ...
        if ollama list 2>/dev/null | grep -q "qwen2.5:3b"; then
            print_step "Model qwen2.5:3b is already downloaded"
        else
            echo ""
            read -p "Download qwen2.5:3b model now? (~2GB) [y/N]: " DOWNLOAD_MODEL
        ...
        if [[ "$INSTALL_OLLAMA" =~ ^[Yy]$ ]]; then
            ...
            print_success "Ollama installed and model downloaded"
        else
            print_warning "Skipping Ollama installation"
            print_warning "Make sure LLM_HOST in .env points to your Ollama server"
        fi
    fi
}
...
"
    ...
    if curl -s "${LLM_HOST}/api/tags" > /dev/null 2>&1; then
        ...
        print_warning "  Make sure Ollama is running before starting the service"
    fi
...
main() {
    ...
    echo "Next steps:"
    echo ""
    echo "  1. Configure FreePBX AudioSocket dialplan"
    ...
    echo ""
    echo "  4. Test by dialing extension 2255 from your payphone"
    echo ""
